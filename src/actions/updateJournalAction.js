...
export function updateJournalAction(userData, journalId) {
    return (dispatch) => new Promise((resolve, reject) => {
        AsyncStorage.getItem(Constant.TOKEN).then((token) => {
            ...
            dispatch({
                error: null,
                saveJournalData: null,
                type: 'SAVE_JOURNAL'
            });
            Axios.put(`${Config.BASE_URL}/journals/user/${journalId}`, userData, config)
                .then((response) => {
                    if (response.status === Constant.HTTP_SUCCESS_CODE) {
                        dispatch({
                            error: null,
                            saveJournalData: response.data.success.data,
                            type: 'SAVE_JOURNAL_SUCCESS'
                        });
                    }
